...
- **LOO-CV**: QCEW likelihood (Student-t) can show high p_loo and k-hat > 0.7 (influential observations) when QCEW precision share is large. The Student-t likelihood and tightened M2 prior mitigate this relative to the original Normal formulation. See `checks.py` for the LOO loop.
